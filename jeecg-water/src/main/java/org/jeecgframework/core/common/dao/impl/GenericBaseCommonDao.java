...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	 ...
	 * 按属性查找不等于某个对象
	 * */
	public <T> List<T> findByPropertyNot(Class<T> entityClass,String propertyName,Object value){
		Assert.hasText(propertyName);
		return (List<T>) createCriteria(entityClass,
				Restrictions.ne(propertyName, value)).list();
	}
...
//		}
	...
	public List<Map<String, Object>> findForJdbcList(String sql) {
		return this.jdbcTemplate.queryForList(sql);
	}
