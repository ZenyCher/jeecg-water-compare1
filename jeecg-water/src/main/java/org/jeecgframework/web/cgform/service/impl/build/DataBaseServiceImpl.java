...
public class DataBaseServiceImpl extends CommonServiceImpl implements DataBaseService{
	...
	public Map<String, Object> findOneForPhone(String tableName,String phone) {
		...
		sb.append(" where member_phone = ? ");
		Map<String, Object> map = commonDao.findOneForJdbc(sb.toString(), phone);
		return map;
	}
	...
	public List<Map<String, Object>> findOneFor(String tableName) {
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer.append("select * from ").append(tableName);
		List<Map<String, Object>> map = commonDao.findForJdbcList(sqlBuffer.toString());
		return map;
	}
	...
	public List<Map<String, Object>> findTableKeyAndValue(String tableName,Map<String, Object> map) {
		StringBuffer sb = new StringBuffer();
		sb.append("select * from ").append(tableName + " where ");
		//循环map得到key和value放入查询条件
		int i = 1;
		for (Map.Entry<String, Object> map1 : map.entrySet()) {
			if( i > 1){
				sb.append(" and " + map1.getKey() + " = ").append("'" +map1.getValue()+ "'");
			}else{
				sb.append(map1.getKey() + " = ").append("'" +map1.getValue()+ "'" + " ");
			}
