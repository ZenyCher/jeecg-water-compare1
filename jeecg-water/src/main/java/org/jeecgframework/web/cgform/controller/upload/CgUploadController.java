...
public class CgUploadController extends BaseController {
	...
	public AjaxJson saveFiles(HttpServletRequest request, HttpServletResponse response, CgUploadEntity cgUploadEntity) {
		...
		/**先根据id和tableName查询该表中该字段是否已有图片，如果有，则在后面增加（以逗号隔开）如果无则新增-开始*/
		try {
			String str = cgUploadService.selectEntryById(id, tableName, cgField);
			if( oConvertUtils.isNotEmpty(str) ) {
				StringBuffer sb = new StringBuffer();
				sb.append(str + "," + cgUploadEntity.getRealpath());
				cgUploadService.writeBack(id, tableName, cgField, fileKey, sb.toString());
			}else {
				cgUploadService.writeBack(id, tableName, cgField, fileKey, cgUploadEntity.getRealpath());
			}
		} catch (Exception e) {
			logger.error("查询字段值为空");
			e.printStackTrace();
		}
		/**结束*/
//		cgUploadService.writeBack(id, tableName, cgField, fileKey, cgUploadEntity.getRealpath());
