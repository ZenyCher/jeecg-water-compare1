...
@Service("wDeviceWaterService")
@Transactional
public class WDeviceWaterServiceImpl extends CommonServiceImpl implements WDeviceWaterServiceI {
 	...
 	public Map<String, Object> selectDeviceWaterById(String deviceId) throws Exception {
 		...
 		sb.append("select * from w_device_water where device_id = ? ");
	 ...
	 * @return
	 */
	private void doAddBus(WDeviceWaterEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(WDeviceWaterEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(WDeviceWaterEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(WDeviceWaterEntity t){
		...
		map.put("device_water", t.getDeviceWater());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,WDeviceWaterEntity t){
 		...
 		sql  = sql.replace("#{device_water}",String.valueOf(t.getDeviceWater()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("w_device_water",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
